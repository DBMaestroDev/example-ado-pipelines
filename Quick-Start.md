...
4. **Customize the `extends: parameters:` block in both thin wrapper YAMLs** (`example-ado-source-control/source-control-workflow.yml` and `example-ado-source-control/ad-hoc-workflow.yml`) for **your** organization and **this** DBmaestro project — none of the values in section 1's table are shared defaults, they're this guide's own example deployment:
   ...
   - `sourceRepoName` (ad-hoc wrapper only) → this alone is **not** enough to repoint `build-git-changes.yml` at your repo: its `resources.repositories` entries for source repos are static (Azure Pipelines doesn't allow repository resources to be parameterized), so you must also edit/add an entry directly in `build-git-changes.yml` for your org/repo and service connection name. `sourceRepoName` is just the repo name (the org is fixed at `DBMaestroDev` in this example — change the `name:`/`endpoint:` in each entry if yours differs), and it's used directly as the `checkout:` alias in the `PrepareSources` stage, so **the repository resource's alias must equal its actual repo name** for this to resolve. `build-git-changes.yml` ships with two example entries (`example-ado-source-control` and `example-ado-source-control-2`) showing that pattern. Keep the wrapper's `sourceRepoName` parameter matching one of the declared aliases — an unrecognized value fails the pipeline compile, to avoid silently building the wrong repo.
