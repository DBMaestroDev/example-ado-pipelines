...
- Pipeline **"Build with Git Change Detection"** (`build-git-changes.yml`) — org `dbmsc`, project `Example`, definition ID **62**. Used by the ad-hoc flow only — see `ad-hoc-workflow.md`. Accepts an optional `packageNames` parameter that, when set, skips its own git-diff detection (the ad-hoc workflow passes this in directly, having already detected changes itself). The ad-hoc workflow also explicitly passes `projectName` (from its own `dBmaestroProjectName` parameter) rather than relying on this pipeline's `'Example-ADO'` default. Which repo it detects changes in and checks out is itself parameterized — `sourceRepoName`/`sourceRepoRef` (defaulting to this guide's own `example-ado-source-control` example, org fixed at `DBMaestroDev`) select the matching entry in its `resources.repositories` block (each aliased to its own actual repo name); the ad-hoc workflow's wrapper sets these to your actual source-control repo (see `ad-hoc-workflow.md` section 2).
