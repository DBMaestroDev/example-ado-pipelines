...
- Pipeline **"Upgrade Environment"** — org `dbmsc`, project `Example`, definition ID **60**. All of its parameters (`targetEnvironment`, `packageNames`, `tagName`, `projectName`, `agentJarPath`, `runnerPool`) have defaults; both workflows explicitly set `targetEnvironment`, `packageNames`, `projectName` (from their own `dBmaestroProjectName` parameter), and `runnerPool`. `targetEnvironment` only accepts `'Release Source'`, `'UAT_Env_1'`, `'Pre_Prod_Env_1'`, or `'Prod_Env_1'` (case-sensitive) — the orchestrator must pass those exact strings.
